...
| iOS bundle | `be.prilleven.mobileapp`, ascAppId `6762270908`, buildNumber `47` (v2.12.0) |
| Android pkg | `be.prilleven.mobileapp`, versionCode `51` (v2.12.0) |
...
├── index.ts                         expo entry
├── app.json                         expo config (version 2.12.0, permissions in NL)
...
### Versionering
- `app.json.expo.version` = user-facing string (huidig `2.12.0`).
- `app.json.ios.buildNumber` (`47`) + `app.json.android.versionCode` (`51`) bumpen bij elke store-release. EAS productie heeft `autoIncrement: true`.
...
| `…/api/community/*` | **(toekomst)** polls, foto's, reply-likes, rapporteren, notificaties | `services/community.ts` (uitbreiding) |
| `…/api/chat-rooms` GET | Roomlijst (gesorteerd op `sort_order`) | `services/chatRooms.ts::listRooms` |
| `…/api/chat-rooms/:slug` GET | Eén room + topics (gepind eerst) + `admin_intro` | `services/chatRooms.ts::getRoom` |
...
4. 🟡 **Community-tijdlijn** — posts, replies, likes, polls, foto's, notificaties.
   ...
   - ⬜ Open: foto-upload bij post, polls, reply-likes, rapporteren, in-app notificaties, categorie-filterbalk.
