...
| Push + app-icoon-badge | `expo-notifications` `~0.32.17` + `expo-device` + `expo-constants` |
| Klembord (kortingscodes) | `expo-clipboard` `~8.0.8` |
...
| iOS bundle | `be.prilleven.mobileapp`, ascAppId `6762270908`, buildNumber `68` (v3.1.1) — EAS `autoIncrement` bumpt dit bij de productiebuild |
| Android pkg | `be.prilleven.mobileapp`, versionCode `74` (v3.1.1) — EAS `autoIncrement` bumpt dit bij de productiebuild |
...
└── src/
    ...
    │                                AanraderLabels/Code/Knop/Relatie),
    │                                ChatFeedback (duimen + reden-venster HapjesHeld),
    │                                RenameModal (titel van een gesprek wijzigen)
    ├── navigation/                  RootStack, LandingTabs (footer 3 tabs),
    │                                pushRouting.tsx (navigationRef + PushRouter:
    │                                tik op een push → Tijdlijn of ChatTopic),
    ...
    │                                familyLayer.ts (recept-family-layer: allergeen-
    │                                normalisatie + per-kind evaluatie + recept-leeftijd:
    │                                getRecipeMinAge / getRecipeAgeLabel / AGE_FILTER_OPTIONS)
    ├── constants/                   theme, data, ingredientIcons,
    │                                links (CHECKOUT_URL / WEBSITE_URL / PRIVACY_URL / TERMS_URL
...
**Toegangscontrole (v3.2.0).** `AppGate` roept ook `useSubscriptionGate(user)` aan (`src/lib/useSubscriptionGate.ts`). Die controleert bij login, elke 60 s in de voorgrond en bij elke `AppState 'active'` of het lidmaatschap nog loopt. **Optimistische start, net als de website**: de eerste render wordt NIET geblokkeerd — de app verschijnt meteen en pas bij een expliciete `active: false` schuift `SubscriptionExpiredScreen` ervoor. Antwoorden die binnenkomen nadat de gebruiker wisselde of uitlogde worden weggegooid (`userRef`-vergelijking). Vóór v3.2.0 controleerde de app dit NERGENS: `allowed_users` werd enkel bij registratie geraadpleegd, dus een verlopen lid hield onbeperkt toegang.
...
├── HapjesHeld → HapjesHeldStack        (Conversations → Chat → RecipeDetail)
...
| `<InfoIconButton onPress />` | open cirkel met groen Feather `info` — lichter van gewicht dan het huisje |
| `<CompactHeader onBack onHome? onInfo? />` | volle header met ChevronBack + rechts optioneel InfoIconButton en/of HomeIconButton |
...
|---|---|
| `AuthScreen.tsx` | 3 tabs (login/register/reset). Whitelist-check vóór signup. Logo + sage/primary kleuren. **Store-compliance (v2.20.0)**: consent-zin op de register-tab ("Door een account aan te maken ga je akkoord met onze gebruiksvoorwaarden en privacybeleid", tappable `Text`-links) + altijd-zichtbare juridische footer (Privacybeleid · Gebruiksvoorwaarden) onder de card. **Kleuren (2026-09-09)**: titel, actieve tab, links, consent-links en de lidmaatschap-CTA staan op `greenText`, conform `.auth-modal` op de web; terracotta blijft enkel voor de knop "Inloggen" (`.btn-primary`) en de wassing van het infovak. Constants `PRIVACY_URL`/`TERMS_URL` → `Linking.openURL` (sinds v3.2.0 uit `constants/links.ts`). **Lidmaatschap-CTA (v3.2.0)**: onder de card, zichtbaar op alle tabs. Op Android een tappable "Word lid van Pril Leven" → `CHECKOUT_URL`; op iOS enkel de neutrale zin "Log in met het account van je Pril Leven-lidmaatschap" — geen link, geen knop, en (sinds 2026-09-18) ook geen verwijzing meer naar wáár je lid wordt. App Store 3.1.1 verbiedt oproepen naar een andere aankoopmethode, en 3.1.3(f) — de regel die ons toelaat zonder in-app aankoop te werken — eist dat er geen enkele oproep tot kopen buiten de app in staat. Zie het commentaar bij `MAG_NAAR_CHECKOUT_LINKEN`. Schakelaar: `MAG_NAAR_CHECKOUT_LINKEN`. **Registratiefout gesplitst (v3.2.0)**: vóór `signUp` draait `checkCanSignUp`; is die false, dan bepaalt `checkAllowedUser` of het "je hebt al een account" of "dit adres is bij ons niet bekend — heb je met dit adres betaald?" wordt. |
| `SubscriptionExpiredScreen.tsx` | **v3.2.0.** Vervangt de hele app zodra de server zegt dat het lidmaatschap niet meer loopt. Spiegel van `showSubscriptionExpiredScreen()` op de website. Toont 🌿 + "Je lidmaatschap is verlopen" + `accessMessage(status)` + optioneel "Je toegang liep tot <datum>". Drie acties: **Check opnieuw** (`recheck()`; blijft het scherm staan, dan wordt de knoptekst "Nog niets gewijzigd — probeer opnieuw"), **Uitloggen**, en op Android **Lidmaatschap verlengen** → `CHECKOUT_URL`. Op iOS staat daar sinds 2026-09-18 een supportroute: "Denk je dat dit niet klopt? Mail ons via hallo@prilleven.be" (tikbare `mailto:`) — géén verwijzing meer naar waar je verlengt, want ook dat is een oproep tot kopen buiten de app (3.1.3(f), zie `MAG_NAAR_CHECKOUT_LINKEN`). Staat bewust BUITEN de `NavigationContainer` — er valt niets te navigeren en de gebruiker mag er niet omheen. |
| `LandingScreen.tsx` | 4 grote tegels (`DraggableTile`), **alleen titel, tekst links-midden** (`tileBg` `justifyContent:'center'`, v2.18.0). Standaard-volgorde: Receptenboek & Weekschema → HapjesHeld 2.0 → Learnings → Allergenen-introductie. **Herordenen (v2.18.0)**: `react-native-draggable-flatlist` + `react-native-reanimated`. Lang drukken op een tegel → `editing`-modus: alle tegels wiebelen (reanimated `withRepeat`/`withSequence` rotatie ±1.3°), elke tegel toont een `move`-handle, en de actieve tegel schaalt 1.04. Sleep → `onDragEnd` → `persistOrder` slaat de key-volgorde op in AsyncStorage `receptenboek_landing_tile_order_<email>`. "Klaar"-knop verlaat edit-modus. In edit-modus navigeert tappen niet. `applyOrder(order)` herschikt `TILES` op opgeslagen keys (onbekende achteraan). **Kop (2026-09-09)**: de losse "INGELOGD ALS"-regel en de titelrij zijn samengevoegd tot één hero-kaart in de stijl van de favorietenzone — groen verloop, zachtgroene rand, titel "Welkom bij Pril Leven" in `greenText`, e-mailadres eronder, `AvatarButton` rechts. De hele kaart is aanklikbaar → `Profile` (uit in sleepmodus, waar de Klaar-knop de plaats van de avatar inneemt). "Kies wat je wil doen" is geschrapt. **Tegels**: alle vijf dragen dezelfde neutrale verdonkering `rgba(0,0,0,0.28)` — het veld `overlayColor` per tegel is verwijderd, zodat er geen kleurzweem meer over de foto's ligt. `useFocusEffect` refresht `community avatar_url`. `navigateFor(key)`: Main / HapjesHeld / Learnings / AllergenenChildren. |
| `ProfileScreen.tsx` | 6 secties: **Account** (e-mail + uitloggen), **Community** (nickname-input met regex-validatie + Opslaan-knop, avatar-blok met `AvatarButton`-preview + "Foto kiezen/wijzigen/Verwijderen", upload-pipeline: `expo-image-picker` → `expo-image-manipulator` resize 512px JPEG q=0.8 → signed Storage URL → `PUT /api/community/profile { avatar_path }`), **Mijn kinderen** (knop → `ChildrenScreen`), **Dieet in het gezin** (9 chips uit `DIET_OPTIONS`, optimistic toggle met 400ms debounce + saveInFlight-ref + pending-queue, status-pill saving/saved/error, flush-on-unmount; vereist bestaand community-profile anders inline hint), **Voorkeuren & privacy** (HapjesHeld memory-toggle via `Switch`, optimistic + rollback, plus knop "Bekijk opgeslagen geheugen →" naar `MemoriesScreen`), **Mijn gegevens** (GDPR-export via `File`/`Paths` + `Sharing.shareAsync`, GDPR-delete via 2-staps modal met `VERWIJDER`-bevestiging), **Over Pril Leven** (v2.20.0 als "Juridisch"; hernoemd 2026-09-18 — "Pril Leven op het web" → `WEBSITE_URL` (prilleven.be, bewust de merksite en NIET de community-webapp met zijn "Word lid"-knop), plus Privacybeleid + Gebruiksvoorwaarden, elk `Linking.openURL` naar `PRIVACY_URL`/`TERMS_URL` op `community-web.prilleven.be`), **Geblokkeerde gebruikers** (v3.0.1, App Store 1.2 — lazy `listBlocks()`, rij per geblokkeerde met nickname + Deblokkeren-knop → `unblockUser`; lege lijst is geen blokkerende fout). Header: `ChevronBack` + titel. Lokale `ChevronBack` om require-cycle met RootStack te vermijden. **Account uitgebreid (v3.2.0)**: rij "Lidmaatschap" (`Actief tot <datum>` / `Actief` / `Niet actief` via `getSubscriptionStatus` + `formatEndDate`) plus een **opzegblok** — intro + knop "Abonnement opzeggen" met `Alert`-bevestiging → `createOpzegverzoek()`. Staat er al een verzoek open (`getOpzegverzoek()` bij mount), dan verdwijnt de knop en verschijnt de bevestigingstekst met datum. Die status komt van de server, niet uit AsyncStorage, dus hij klopt ook na herinstalleren of op een ander toestel. `PRIVACY_URL`/`TERMS_URL` komen sinds v3.2.0 uit `constants/links.ts` in plaats van lokale constanten. |
| `ChildrenScreen.tsx` | Lijst van kinderen (cards met naam + leeftijd via `formatAge`, optionele detail-rows voor bekende allergieën, geïntroduceerde allergenen, eerdere reacties, opmerkingen). Allergeen-labels **zonder emoji** (v3.1.x — `allergenLabel` geeft enkel `opt.label`). De rij **"Reeds geïntroduceerde allergenen"** (v3.1.x, hernoemd van "Geïntroduceerd") toont **"Functie uitgeschakeld"** (muted/italic) wanneer `child.allergens_opted_out`, anders de lijst geïntroduceerde allergenen (web-parity). `useFocusEffect` herlaadt na terugkeer uit `ChildForm`. Edit-knop → `navigate('ChildForm', { childId })`, verwijder-knop → `Alert.alert` confirm → `archiveChild` (soft delete). "Kind toevoegen"-CTA onderaan. Header: `ChevronBack` + titel. **NB v2.6.0**: de allergeen-flow start niet meer hier maar vanaf het landingscherm (Allergenen-tile → `AllergenenChildrenScreen`). |
| `ChildFormScreen.tsx` | Add/edit-formulier voor een kind. Route-param `childId` bepaalt edit-modus (laadt via `getChildren()` + filter — geen aparte GET-by-id endpoint). Velden + validatie (parity met website-UI sinds verwijdering textuur/eczeem): naam (verplicht, max 50), geboortedatum (regex `^\d{4}-\d{2}-\d{2}$`, max vandaag, min 10 jaar terug — v2.8.4: validatie gebruikt lokale getters i.p.v. UTC zodat geboortedatum vandaag/morgen geen timezone-fout geeft), known_allergies (9 chips uit `KNOWN_ALLERGEN_OPTIONS`), previous_reactions (textarea max 1000), notes (textarea max 500). `KeyboardAvoidingView` op iOS. **Allergenen-introductie keuze (Optie B, parity met web `renderKindForm`)**: sectie "Al geïntroduceerde allergenen" met "Alles selecteren/deselecteren"-link + `CheckRow`-grid (`pre_introduced`) + twee elkaar uitsluitende checkboxes ("introductie nog volgen" / "functie uitschakelen"). Bij edit vooraf ingevuld via `getEhState(childId)` (try/catch — throwt als er nog geen state is; dan `prepFollow=!opted_out`). Validatie: minstens 1 vakje OF 1 van de 2 keuzes. Na `createChild`/`updateChild` → niet-blokkerend `patchEhState(childId, { allergen_state: prepDisable ? { opted_out:true, setup_done:true } : { pre_introduced, setup_done:true, opted_out:false } })`. Op succes: `goBack()` → ChildrenScreen herlaadt automatisch. |
| `AllergenenChildrenScreen.tsx` | Kind-picker voor de allergeen-flow (v2.6.0). Bereikbaar via de "Allergenen-introductie"-tile op `LandingScreen`. Laadt `getChildren()` met `useFocusEffect`. Toont per kind een kaart (naam + leeftijd via `formatAge` + chevron); tap → `navigate('EersteHapjes', { childId })`. Bij 0 kinderen: empty-state met CTA naar `Children` om er eerst een aan te maken. Header: `ChevronBack` + titel. **Vernieuwd (2026-09-18)**: hero-kaart met groen verloop ("ALLERGENEN-INTRODUCTIE" / "Voor wie?") en per kind een kaart met een gekleurde initiaalbol uit `lib/familyLayer.ts` (`colorFromSeed` + `initialsFromName`), zodat een kind er door de hele app hetzelfde uitziet. De introtekst zit achter een info-knop in de header (`InfoModal`). Laadt stil: enkel de eerste keer een spinner, daarna blijft de lijst staan. |
| `EersteHapjesScreen.tsx` | Allergenen-grid per kind (v2.5.0 MVP, sterk uitgebreid t/m v2.9.1). Laadt via `useFocusEffect`: `getChildren()` + `getEhState(childId)` + `getEhDoses(childId)`, dan `buildAllergenContext(doses, state, ageMonths)`. Render-stage state-machine spiegelt website `js/components/allergenen.js`: **disabled → pause → welcome → setup → grid**. **DisabledCard** (v3.1.x, parity met `renderDisabled()`): wanneer `allergen_state.opted_out` (gekozen in `ChildFormScreen` via "functie uitschakelen") toont het scherm 🔕 + "Allergenen-introductie staat uit voor X" + "Toch volgen"-knop (`handleReenableFlow` → `patchEhState { opted_out:false, started:false }`); symptoom-knop + HoeveelhedenBox worden dan verborgen. Heeft voorrang op alle andere stages. (1) Intro-text onder header (v2.8.8, parity met `.allergenen-intro`). (2) **WelcomeCard** (v2.8.6): 🍽-icoon + "Start met introduceren"-CTA als `state.started !== true`. (3) **Setup-card** "Reeds geïntroduceerd?" (v2.8.5/v2.8.6): checkbox-grid met foto-achtergrond + `LinearGradient` sage-overlay; persisteert `pre_introduced[]` in allergen-state. (4) **Grid** met 9 tegels — status-badge (Veilig / Bezig / Nog te doen / Allergisch / Gepauzeerd / Nog te jong / Overgeslagen) en `X/3 doses` teller. Tegels gebruiken `ImageBackground` uit `assets/allergens/`. **Pause-flow** (v2.8.9 hide-basis + v2.9.1 volledige wizard): zodra `state.paused === true` worden de Symptoom-loggen-knop én HoeveelhedenBox verborgen. De **pause-wizard** (3-staps modal, parity met website) wordt getoond als `paused && paused_step >= 1`: stap 1 info-kaart ("Gelezen"), stap 2 arts-vraag ("Heb je een arts gecontacteerd?"), stap 3 allergisch ja/nee. Trigger zit in `SymptomFormScreen` (reactie matig/heftig + gelinkt allergeen → `patchEhState` met `paused/paused_type ('twijfel'|'ernstig')/paused_step:1/paused_allergen`). Callbacks `advancePauseStep` / `handleAllergyConfirmed` (markeert allergisch + reset pauze) / `handleAllergyDenied` (reset pauze). Cooldown-banner als laatste dose < `ALLERGEN_COOLDOWN_DAYS` (2 dagen) geleden. Tik op een tegel → `navigate('DoseForm', { childId, allergenKey })`. Locked-age tegels zijn niet-klikbaar. **Arts-toezicht-modus** (v2.9.0/v2.9.1): als `state.arts_toezicht === true` krijgt elke nog-niet-veilige/niet-allergische tegel een "Overslaan"-knop (text-only, rechtsonder absolute positioned binnen de ImageBackground, zodat de foto over de volledige gesloten tegel blijft). Klik → `handleToggleExclude` patcht `allergen_state.excluded_keys` (Set-toggle); status wordt `'excluded'` (🚫 Overgeslagen) en knop verandert in "Opnemen". Onderaan een "Symptoomlog →"-knop → `navigate('SymptomLog', { childId })`. Header: `ChevronBack` + titel. **Weergave v3.2.0 (web-pariteit met `renderAllergenPath` + `renderSafetyBar`)**: (a) **`AllergenPathCard`** boven de tegels — zachtgroene kaart met `Allergenenpad · <kindnaam>`, teller `x/9` opgevolgd (veilig + gekende allergie tellen mee), negen klikbare statussegmenten (safe/known/active/paused/neutral, kleuren uit `STEP_STYLE`), een korte mijlpaal ("Eerste allergeen afgerond" / "Allergenenpad afgerond" — vooraf aangevinkte en gekende allergenen tellen daar NIET voor mee), en de volgende introductie met allergeenfoto + gradient, hoeveelheid-pil (`DOSE_AMOUNT_TEXT`), suggestie en CTA. Een tik op een segment opent die tegel en scrolt ernaartoe (`focusAllergen`: `openKey` zetten, 260 ms wachten op de accordeon-animatie, dan `scrollTo` met `listYRef` + `tileYRef` uit `onLayout`). (b) **Losse `HoeveelhedenBox` verwijderd** — de hoeveelheid staat nu in de Volgende stap-kaart, waar ze relevant is. (c) **Arts-toezicht-banner verwijderd** — staat nu als compacte chip onder de CTA (`pathSupervision`). (d) **`SafetyBar`** vervangt de oude `artsWarn`: maximaal ÉÉN melding per kindje, niet inklapbaar, twee niveaus via `safetyLevel(doses, symptoms)` — ernstig (dose `ernstig` of symptoom `heftig`, rood #fdecec) verdringt twijfel (dose `mild` of symptoom `matig`, geel #fff9e7). Kijkvenster 14 dagen. Statusteksten worden niet herhaald; die blijven in de tegels. **Compacter + sneller (2026-09-18)**: de twee introteksten en de uitleg over medisch toezicht zitten achter een **info-knop** in de header (lokale `InfoIconButton`, want importeren uit RootStack geeft een require-cycle). Het kind-blok is geschrapt — de naam staat al in "Allergenenpad · <naam>" — en **Symptoom loggen** is een icoon in de header geworden. In de Allergenenpad-kaart is de losse CTA weg: de **hele Volgende stap-kaart is de knop**, en "🩺 Medisch toezicht" staat als pil naast het label VOLGENDE STAP i.p.v. op een eigen regel. Laadt via **`getEhOverview`** (één verzoek voor state + doses + symptomen) parallel met `getChildren()`; daarvóór wachtte alles op de kinderenlijst en stonden de drie aparte functions koud (gemeten 3762 ms voor doses alleen). |
| `DoseFormScreen.tsx` | Modal-style add-dose form. Route-params `{ childId, allergenKey }`. Laadt bestaande doses via `getEhDoses(childId, allergenKey)`, suggesteert volgende vrije `dose_number` via `nextDoseNumber()`. Velden: dose nummer (3 chips 1-2-3, "al gedaan"-hint op bezette posities), reactie (3 radio-cards geen/mild/ernstig met description; bij ernstig een danger-banner "arts contacteren"), `intro_date` (TextInput jjjj-mm-dd, default `todayIsoDate()`, niet in toekomst), notes (textarea max 500). Save → `createEhDose()`. Op succes goBack → EersteHapjesScreen herlaadt. Header: `ChevronBack` + titel. **2026-09-20**: leest de doses uit de gecachete `getEhOverview(childId)` en filtert zelf op `allergen_key`, i.p.v. een eigen `/doses`-call die sinds de bundeling bijna altijd koud stond. |
...
| `RecipeListScreen.tsx` | Zoekbalk + filterpanel (eetmoment + allergeen chip-rijen). `Promise.all`-load. `FlatList` met `RefreshControl`. **Multi-select filter (2026-06-04)**: `momentFilter`/`allergenFilter` zijn nu `string[]` i.p.v. één waarde — meerdere eetmomenten én meerdere allergenen tegelijk. Eetmomenten = "minstens één van" (recept matcht als het één geselecteerd moment bevat); allergenen = "wegfilteren bij minstens één" (recept verborgen als het één geselecteerd allergeen bevat). Allergeen-chips komen uit de canonieke **9** `KNOWN_ALLERGEN_OPTIONS` (met icon), recept-allergenen via `normalizeAllergen` (legacy→canoniek) zodat ze matchen. Reset-chips ("Alle eetmomenten" / "Geen allergenen filter") wissen de hele selectie. **Leeftijdsfilter (web-parity 3.1.x)**: derde chip-rij in hetzelfde filterpanel — `ageFilter: number[]` + reset-chip "Alle leeftijden" + één pill per `AGE_FILTER_OPTIONS` (`[6,7,9,10]` uit `familyLayer`). OF-logica: recept zichtbaar als `getRecipeMinAge(r)` in de selectie zit; telt mee in `filtersActive` (filter-dot). |
| `RecipeDetailScreen.tsx` | Foto, fav-toggle, info-tags, **portion-scaling** o.b.v. actief weekschema (`X = ceil(persons/portions)`), ingredients, steps, sterren + comments. **Voor jouw gezin (family-layer, web-parity)**: laadt `getChildren()` mee in de `Promise.all` (`.catch(()=>[])`, niet-blokkerend). `FamilyLayer`-component evalueert elk kind via `evaluateFamily(recipe, children)` uit `src/lib/familyLayer.ts` (status rood→allergisch / jong→te jong / oranje→nog niet geïntroduceerd / ok); toont avatar-rij (gekleurde initiaal-bol + status-emoji + naam + leeftijd) en gekleurde waarschuwingsboxen per kind met een waarschuwing. Kinderen met `allergens_opted_out` tellen niet mee voor "nog niet geïntroduceerd". `getRecipeMinAge` valt terug op meal-moment-minima als `recipe.minAgeMonths` ontbreekt. Render null bij 0 kinderen. **Leeftijd-badge (web-parity 3.1.x)**: grotere groene pill-badge `getRecipeAgeLabel(recipe)` ("vanaf X mnd") rechts naast de "Informatie"-titel (`sectionTitleRow` draagt de onderlijn). Niet getoond bij onbekende leeftijd. **Babyhapje-tips**: `BabyPrepTips`-component (inklapbaar, "Kinderhapje maken — papje of stukjes") onderaan de bereidingswijze. **Allergenen-tags genormaliseerd (2026-06-04)**: de info-tag-rij toont `getAllergenLabel(normalizeAllergen(a))` met dedupe via `Set` (i.p.v. rauwe DB-waarde), zodat legacy-waarden (gluten/lactose/ei) altijd als canonieke labels (Tarwe/Koemelk/Kippenei) verschijnen. |
| `WeekScheduleScreen.tsx` | 2 sub-tabs (`active` / `generate`), 3 presets (`today` / `today-tomorrow` / `week`). Genereer-knop + per-slot 🔄 refresh. Modal "Opslaan met naam". **Sub-tabs (2026-09-09)**: twee kaarten naast elkaar in de stijl van de favorietenzone (groene wassing + rand bij de actieve), elk met een titel en een hint — links de naam van het actieve schema, rechts "Nieuw schema samenstellen". Bewuste AFWIJKING van de web, waar dit platte teksttabs met een onderlijn zijn. De dagkiezer blijft een segmented control (`.day-selector-bar`). **Weergave (2026-09-09)**: de genereer-tab toont het schema als `ScheduleTable` (web-pariteit met `renderScheduleGrid`), de actieve tab als `ActiveDayBlocks` (web-pariteit met `renderActiveDays`) — net als op de website twee verschillende weergaven. **Favorietenvoorkeur (2026-09-09)**: knop "Gebruik favorieten" rechts naast de titel "Allergenen uitsluiten"; keuze wordt bewaard als `preferFavorites` in het schema. Favorieten worden bij focus herladen (`useFocusEffect`), want dit scherm blijft in de tabbalk gemonteerd. **Uitleg (2026-09-09)**: het introblok is vervangen door de info-knop in de header (`InfoModal`), enkel zichtbaar in de genereer-tab. De koppen "Weekschema Generator" en de schemanaam boven de dagkiezer zijn geschrapt — de sub-tabs zeggen het al. |
...
| `FavoritesScreen.tsx` | **Herbouwd naar web-pariteit (2026-09-09)** met `js/components/favorites.js`. Hero-kaart (`expo-linear-gradient`) met "VOOR LATER BEWAARD" + "Jouw favorieten", daaronder twee **tel-tabs** (`.favorites-summary-item`): aantal recepten en aantal weekschema's, elk met een groot groen getal. Per tab één paneel, nooit allebei; **recepten staat vooraan** (`activeFavoritesTab = 'recipes'` op de web). Schemakaarten volgen `.saved-schedule-card`: labeltje WEEKSCHEMA + ronde Actief-badge, naam, "Opgeslagen <datum>" via een lokale `formatDateShort` (spiegel van `js/utils.js`), rechts een personen-pil; actierij met één gevulde groene hoofdknop (Boodschappenlijst of Activeren), omrande secundaire knoppen en Verwijderen als kale rode tekst. Details tonen `ScheduleTable`, net als `renderScheduleDetail`. Daarvoor bewaart het scherm de volledige recepten uit `getRecipesByIds` in plaats van enkel de namen. Lege staten volgen `.favorites-empty-state` (gestippelde groene rand). De uitleg over de boodschappenlijst zit in de header-info-knop (`InfoModal`), enkel in de weekschema-tab. **Bewust NIET overgenomen**: de knop "Bewerken" — een opgeslagen schema aanpassen bestaat niet in de app. |
| `HapjesHeldScreen.tsx` | RAG-chat met `UsageBar` (maand-€-budget), foto-counter (`remaining/limit`), `HealthDisclaimerModal`, link-parser voor assistant-tekst. **Web-pariteit 2026-09-26**: (a) **streaming** — het antwoord verschijnt terwijl het geschreven wordt (`streamingText` in de footer-bubbel); het definitieve `answer` uit `done` vervangt het, want de server haalt daar ongeldige receptlinks nog uit. Meescrollen enkel als de gebruiker onderaan staat (`nearBottomRef` via `onScroll`, `scrollToEnd` zonder animatie). (b) **Receptlinks** (`recipeIdFromUrl`) openen `RecipeDetail` binnen de HapjesHeld-stack, full screen; terug = het gesprek. Andere links blijven naar de browser gaan. Receptlinks bewust NIET naar de browser: ze wijzen naar `community-web`, de site met de "Word lid"-knop, en dat mag op iOS niet (zie `MAG_NAAR_CHECKOUT_LINKEN`). (c) **Duim omhoog/omlaag** per antwoord (`ChatFeedbackRow`, enkel op berichten met een DB-id `feedbackId`); bij duim omlaag opent `FeedbackReasonModal` met een optionele reden. Opgeslagen feedback wordt bij het openen van een oud gesprek meegeladen. (d) **Fototip** in het welkomstbericht (koelkast/voorraadkast) + placeholder "Typ je vraag of voeg een foto toe" — zonder de 📷 die de web in beide heeft (§8, geen emoji). |
| `ConversationsScreen.tsx` | Gesprekkenlijst met `useFocusEffect` + `RefreshControl`. **Vernieuwd (2026-09-26)**: hero-kaart met groen verloop ("HAPJESHELD" / "Jouw gesprekken" + aantal + terracotta knop Nieuw gesprek), witte kaarten met zachtgroene rand, `SectionList` gegroepeerd in Vandaag / Afgelopen week / Eerder. **Verwijderen in bulk**: "Selecteer" rechts in de stack-header of lang drukken op een gesprek → selectiemodus met vinkjes, onderaan een actiebalk "Alles selecteren" + rode "Verwijder (n)" met één bevestiging. Er is geen bulk-endpoint: `deleteConversation` per id via `Promise.allSettled`; mislukte blijven aangevinkt met een toast. Stil herladen na de eerste keer. **Titel wijzigen**: potlood-knop per gesprek → `RenameModal` (`components/RenameModal.tsx`) → `renameConversation` (PATCH, max `CONVERSATION_TITLE_MAX` 80). Zelfde titel als in de zijbalk op de website; de server overschrijft een eigen titel nooit (automatische titel enkel als er nog geen is). |
| `TimelineScreen.tsx` | Community-tijdlijn MVP (v2.10.0), tweede footer-tab. `FlatList` met cursor-paginatie via `before` (= `created_at` van laatste niet-gepinde community-post, berekend in `computeCursor`), `RefreshControl` + `useFocusEffect`-refresh + `onEndReached`-loadMore (dedupe op id-Set), `PAGE_SIZE = 20`. **Gevolgde chatruimte-topics in de feed (v2.15.0)**: de server merget gevolgde chatroom-topics (`source_type === 'chatroom'`, met `title`/`source_room_title`/`source_room_slug`) al in `GET /api/community/posts`; deze worden NIET meer weggefilterd maar gerenderd als **`ChatroomTopicCard`** (sage linker-rand, bron-badge "Chatruimte: …", titel, body-snippet 200 tekens, footer "X reacties · Open discussie"). Tik → cross-tab `navigation.navigate('Chatruimtes', { screen: 'ChatTopic', params: { topicId, roomTitle } })`. `computeCursor` blijft community-only (chatroom-items pagineren mee op dezelfde `before`). **`PostCard`** is zelf-bevattend: eigen like-state (optimistic toggle met rollback via `togglePostLike`), lazy-loaded replies (`listReplies` bij eerste uitklap) + inline reply-composer (`createReply`), pin-badge ("Mededeling"), admin-badge, categorie-chip (verborgen voor `algemeen`), optionele foto. **`Composer`** (alleen admins, in `ListHeaderComponent`): tekst + categorie-pills uit `POST_CATEGORIES` → `createPost`. Admin-check via `getIsAdmin(user)`. `PostAvatar` toont foto of gekleurde initiaal-bol. **Eigen content bewerken/verwijderen** (v2.12.0, via `getCurrentUserId()` vergeleken met `user_id`): eigen post krijgt `edit-2`/`trash-2`-iconen in de kop → bewerken = inline `TextInput` (`editPost`, max `POST_BODY_MAX`), verwijderen = `Alert` confirm → `deletePost` → `onPostDeleted` filtert uit de feed. Eigen reply idem: inline edit (`editReply`, max `REPLY_BODY_MAX`) + delete via `Alert` → `deleteReply` (rechtstreeks uit `services/community` wegens naam-botsing met `chatRooms`). Geen edit-window (server doet owner-check, 403 bij niet-eigenaar). **Reply-likes** (v2.12.1): elke reactie heeft een hartje + teller onder de body (optimistic toggle via `toggleReplyLike`, rollback bij fout; teller verborgen bij 0). **Moderatie — App Store 1.2 (v3.0.1)**: op posts/replies van een ánder lid een `more-horizontal` (···)-knop → `openModerationMenu` (`src/lib/moderation.ts`) met *Rapporteren* (`reportCommunityTarget`) + *Gebruiker blokkeren* (`blockUser` → `onUserBlocked` verwijdert de content lokaal uit de feed). Niet zichtbaar op eigen content (daar staan edit/trash). Buiten scope: foto-upload, polls, notificaties. **Sneller (2026-09-18)**: bij het openen liepen **twee** volledige feed-verzoeken tegelijk (`useEffect` én `useFocusEffect`). Nu één; de eerste focus toont de spinner, daarna wordt stil ververst met behoud van de feed. De landing roept `prefetchFeed(FEED_PAGE_SIZE)` aan, die de tijdlijn bij zijn eerste load overneemt via `takePrefetchedFeed`. **Bewust geen cache** op de feed: likes en reacties wijzigen de tellers, en een bewaarde feed zou een hartje tonen dat je net had uitgezet. |
| `ChatRoomsListScreen.tsx` | Chatruimtes-tab root (v2.11.0). Toont de 4 vaste rooms via `listRooms()` (server leidend; valt terug op lokale `ROOMS`-constant zodat de lijst nooit leeg is). Per room een kaart met titel + optionele beschrijving + chevron (**emoji-bol verwijderd in v2.17.1**). **Stille focus-reload (v2.17.1)**: eerste focus toont de centrale spinner (`initial`), daarna stil herladen (`silent`, geen `RefreshControl`-cirkel) via een `loadedOnce`-ref — voorkomt de vastzittende ververs-cirkel; `refresh`-mode enkel nog voor pull-to-refresh. **Per-room badge (v3.1.0)**: de rode badge per room toont nieuwe ADMIN-activiteit (nieuwe topics + replies) sinds de chatruimtes laatst geopend werden — dezelfde bron als de footer-badge via `chatroomRoomCounts[item.id]` uit `NotificationContext` (niet meer `getUnread()`). Voor admins tellen alle leden + admin mee. Bij focus roept `load()` ook `refresh()` aan zodat de tellers meteen kloppen. Dit staat los van het "volgen" van een room: een gevolgde room zonder badge toont nog steeds de sage gevolgd-stip (`item.is_followed`). **Stille focus-reload (v2.17.1)**: eerste focus toont de centrale spinner (`initial`), daarna stil herladen (`silent`). Tap → `navigate('ChatRoom', { slug, title })`. `useFocusEffect` + `RefreshControl`. Eigen header (geen stack-header). **Vernieuwd (2026-09-18/20)**: hero-kaart met "CHATRUIMTES" / "Waarover wil je praten?", in dezelfde stijl als de kind-picker. De vier ruimtes komen **meteen** uit de lokale `ROOMS`-constant en de server vult stil aan — daarvoor zat de lijst achter een spinner tot de (vaak koude) chat-rooms-function antwoordde. `keyExtractor` op `slug`, want lokale ruimtes hebben de slug als id en die van de server een UUID. Initiaalbollen zijn er bewust NIET: bij vier vaste ruimtes voegen ze niets toe. |
| `ChatRoomScreen.tsx` | Topic-lijst van één room (v2.11.0). `getRoom(slug)` → `{ room, topics }`. Optioneel **admin-welkomsbericht read-only** bovenaan (`room.admin_intro`, niet bewerkbaar op mobiel). `FlatList` van topics (gepind-badge, titel, body-snippet, auteur-avatar + nickname + admin-badge, `relTime(last_reply_at||created_at)`, replies-teller). Floating "Nieuw topic"-knop → `navigate('ChatTopicForm', { slug })`. Tap topic → `navigate('ChatTopic', { topicId, roomTitle })`. **Per-topic notificatiebadge (v3.1.0)**: elke topic-card toont rechtsboven een rode badge (`chatroomTopicCounts[item.id]` uit `NotificationContext`, "99+"-cap) met de nieuwe admin-activiteit (nieuw topic + nieuwe replies) sinds dat topic laatst geopend werd; voor admins tellen alle leden mee. `useFocusEffect` roept `refresh()` (NIET meer `markChatroomsSeen`) zodat de badges meteen kloppen — het openen van een room wist de tellers niet langer; een topic-badge verdwijnt pas wanneer je dát topic opent (`markTopicSeen` in `ChatTopicScreen`). **Volg-knop (v2.14.0)**: header-rechts toggle "Volg"/"Gevolgd" (`navigation.setOptions` headerRight, plus/check-icoon) → `followRoom`/`unfollowRoom` optimistisch met rollback; bij openen van een gevolgde room én na volgen wordt `markRoomRead(slug)` aangeroepen zodat de ongelezen-badge reset. `useFocusEffect` + `RefreshControl`. Stack-header met `ChevronBack` + room-titel. **2026-09-20**: `isFollowed` is `boolean | null`; de volgknop verschijnt pas zodra de status bekend is. Daarvoor stond er eerst "Volg" en klapte hij om naar "Gevolgd" zodra de server antwoordde. Het plusje was terracotta terwijl rand en tekst groen zijn — nu `greenText`. |
| `ChatTopicScreen.tsx` | Topic-detail + reacties (v2.11.0). `getTopic(topicId)` → `{ topic, replies }`. `TopicHeader` (titel + body + auteur) en `ReplyRow`'s in een `FlatList` (`ListHeaderComponent`). **Per-topic badge wissen (v3.1.0)**: `useFocusEffect` roept `markTopicSeen(topicId)` (`NotificationContext`) aan → wist de per-topic notificatiebadge in `ChatRoomScreen` en laat de room-/footer-tellers krimpen (los van het follow-based `markTopicRead`). **Eigen content** (via `getCurrentUserId()` vergeleken met `user_id`): Bewerken-knop alleen binnen 15 min (`isWithinEditWindow`), Verwijderen altijd. Topic-edit → `navigate('ChatTopicForm', { topicId, initialTitle, initialBody })`; topic-delete → `Alert` confirm → `deleteTopic` → goBack. Reply-edit = inline `TextInput` (`updateReply`); reply-delete = `Alert` → `deleteReply`. Onderaan reply-composer (`createReply`, geïmporteerd rechtstreeks uit `services/chatRooms` wegens naam-botsing met `community.ts`). `KeyboardAvoidingView` met `keyboardVerticalOffset={useHeaderHeight()}` (v2.17.1, `@react-navigation/elements`) zodat de composer exact boven het toetsenbord uitlijnt i.p.v. een hardcoded offset. **Moderatie — App Store 1.2 (v3.0.1)**: op een topic/reply van een ánder lid een vlaggetje-knop "Rapporteren / blokkeren" → `openModerationMenu` (`src/lib/moderation.ts`) met *Rapporteren* (`reportChatTarget`) + *Gebruiker blokkeren* (`blockUser` uit `services/community`, gedeelde `user_blocks`). Niet zichtbaar op eigen content. Stack-header `ChevronBack`. **2026-09-20**: zelfde behandeling van de volgknop als in `ChatRoomScreen` (`isFollowed: boolean | null`, groen plusje). |
...
| `AanradersScreen.tsx` | **v3.2.0.** Native tegenhanger van de publieke pagina `/aanraders`. Leest via `getAanraders()` rechtstreeks uit Supabase (anon-key mag: RLS `select using (zichtbaar = true)`), dus GEEN website-endpoint. Opbouw als de website: intro met transparantietekst (`INTRO_TEKST`), zoekveld, filterpillen, per categorie een sectie met `AanraderKaart`, en de gratis downloads. **Filterregel** (spiegel van `pillGroep`): een filtergroep verschijnt pas als minstens twee opties elk minstens twee producten hebben — met de huidige data toont enkel *Categorie*, en blijven *Merk* en *Leeftijd* verborgen. Leeftijd is een **ondergrens**, dus enkelvoudig. Categorieën met `binnenkort = true` tonen het opbouw-blok en hun producten worden overal weggelaten (`beschikbaar`), ook uit de filters en de zoekresultaten. Géén emoji voor de categorienaam (CLAUDE.md §8 + web). **Geen apart categoriescherm** zoals `/aanraders/c/<slug>`: die pagina's bestaan op de web vooral voor Google, de categoriefilter doet hier hetzelfde. |
| `AanraderProductScreen.tsx` | **v3.2.0.** Productdetail (`/aanraders/p/<slug>`). Haalt het product uit dezelfde gecachete `getAanraders()`-call, dus meestal zonder extra request. Toont foto, labels, merk, titel, chips (leeftijd/materiaal/categorie), korte + lange beschrijving, "Waarom ik dit aanbeveel", opmerking, voordelen, nadelen en FAQ — elk blok enkel als het veld gevuld is (lange_beschrijving, nadelen, faq en afbeeldingen staan vandaag leeg in de DB). Onderaan kortingscode + koopknop + transparantieregel. De gedeelde bouwstenen (`AanraderLabels`, `AanraderCode`, `AanraderKnop`, `AanraderRelatie`) staan in `components/AanraderKaart.tsx`, zodat kaart en detail niet uiteen kunnen lopen. `AanraderCode` kopieert via `expo-clipboard` en toont 1,8 s "Gekopieerd ✓" — zelfde bevestiging als `kopieerCode()` op de website. |
| `LearningsScreen.tsx` | Learnings-bibliotheek (v2.17.0), read-only. Laadt alle items via `getLearnings()` (geen server-filter) met `useFocusEffect` + `RefreshControl`. Client-side `filtered` useMemo: type-filter (KIND_FILTERS chips Alle/Blog/Document/Video) + favorieten-only (hart-chip) + zoeken over titel/beschrijving/tags. `FlatList` van kaarten: thumbnail (`Image` of emoji-placeholder via `learningKindIcon`), type-badge + duur (`formatDuration`), titel, beschrijving, hartje-favorietknop (optimistic toggle met rollback via `toggleLearningFavorite`). **Tap-gedrag (v2.18.0)**: documenten (`kind==='pdf'`) → `navigate('LearningPdf', { id, title })` (eigen PDF.js-viewer met bladwijzer-sync); blog/video → `navigate('LearningDetail', { id, title })`. (WebBrowser/openingId verwijderd in v2.18.0.) Lokale `ChevronBack` + titel "Learnings". Empty-state met `book-open`-icoon. **Mijn leertraject (v3.2.0, gamification fase C)**: boven de lijst een `LeertrajectKop` met "X afgerond · Y bezig" en een "Ga verder met …"-knop naar het item met de recentste bladwijzer. Elke kaart draagt een statusbadge Nieuw / Bezig / Afgerond. **Bezig** wordt afgeleid van `learning.bookmark.position` — serverdata, dus gedeeld met de website. **Afgerond** staat lokaal in AsyncStorage (`lib/learningProgress.ts`) en synchroniseert bewust NIET met de web; dat wacht op fase 3 van het gamificatieplan (Supabase-tabel met RLS). **Sneller (2026-09-18)**: `getLearnings()` zit achter een cache van 30 s (gewist door `toggleLearningFavorite` en `putLearningBookmark`, want die bepalen het hartje en de badges); pull-to-refresh omzeilt hem via `{ force: true }`. Enkel de eerste keer een spinner. De landing haalt de bibliotheek bij het inloggen eenmalig op de achtergrond op, wat de cache vult én de Vercel-function warmt. |
| `LearningDetailScreen.tsx` | Learning-viewer (v2.17.0), read-only. `getLearning(id)` in `useEffect` (goBack bij fout). Header: lokale `ChevronBack` + titel + favoriet-hartje (optimistic + rollback). Render per `kind`: **blog** → `WebView` (`react-native-webview`) met `wrapBlogHtml(body_html)` (theme-gestylede HTML-wrapper); **video** → `expo-video` `VideoView` + `useVideoPlayer(signed_url)` (nativeControls, allowsFullscreen, contentFit contain) in ScrollView; **pdf** → kaart met "Document openen"-knop → `WebBrowser.openBrowserAsync(signed_url)` (legacy-fallback; pdf-navigaties komen sinds v2.18.0 via `LearningPdfScreen`, niet meer hier). **Video-positie sync (v2.18.0)**: bij `kind==='video'` haalt het scherm `getLearningBookmark(id)` op (→ `resumeRef`); zodra de player `statusChange`→`readyToPlay` triggert, springt hij eenmalig naar `resumeRef` (`player.currentTime`). Een `timeUpdate`-listener (interval 2s) slaat de positie debounced (1500ms) op via `putLearningBookmark(id, { seconds })` (skip als < 1s verschil met `lastSavedRef`); op cleanup wordt de laatste positie geflusht. Parity met website-bookmark `seconds`. Notities/clips van de website blijven buiten de mobiele app. **Afrondbalk (v3.2.0)**: onderaan een vaste balk "Klaar met deze learning?" met een knop die de lokale afgerond-status zet, en daarna "Afgerond ✓ · Ongedaan maken". De website gebruikt hier een schuifbeweging; op mobiel is een knop met undo even bewust en voorspelbaarder. Dezelfde balk staat in `LearningPdfScreen`, want documenten openen daar en niet via dit scherm. |
| `LearningPdfScreen.tsx` | Eigen **PDF.js-viewer** in een `WebView` (v2.18.0), full-screen, read-only. Route-params `{ id, title? }`. Laadt `Promise.all([getLearning(id), getLearningBookmark(id)])`, bepaalt `startPage = bookmark?.page_nr ?? 1`, en bouwt HTML via `buildViewerHtml(signed_url, startPage)`: laadt pdf.js 3.11.174 van cdnjs, rendert alle pagina's als canvases (body-breedte × devicePixelRatio), `scrollIntoView` naar `START_PAGE`, en een scroll-listener (250ms) detecteert de pagina dichtst bij het midden → `window.ReactNativeWebView.postMessage({type:'page', page})`. `onMessage`: type `page` → `pageRef` + debounced (`SAVE_DEBOUNCE_MS` 1500) `savePage` → `putLearningBookmark(id, { page_nr })` (skip als gelijk aan `lastSaved`); type `error` → toast. Cleanup flusht de timer + slaat `pageRef` op. **Bladwijzer-sync met de website**: page_nr wordt gedeeld via dezelfde `/api/learnings/:id/bookmark`, dus een bladwijzer op de web-app brengt je op de gsm op dezelfde pagina (en omgekeerd). Geen download-knop (parity: documenten downloaden gebeurt enkel op de website). Lokale `ChevronBack` + titel. **Lui renderen (2026-09-18)**: eerst worden alleen lege canvassen met de juiste afmetingen geplaatst; pas wat rond de zichtbare pagina ligt wordt gerasterd (venster van 3, één render tegelijk), en pagina's ver uit beeld worden weer leeggemaakt zodat het geheugen niet oploopt. De pixelratio is afgetopt op 2. Daarvoor werd **elke** pagina vooraf gerenderd, wat bij een dik document minutenlang duurde. |
...
**Routes** in `RootStackParamList`: `Landing` (→ `LandingTabs`: Functies/Tijdlijn/Chatruimtes; Chatruimtes = `ChatRoomsStack`: RoomList/ChatRoom/ChatTopic/ChatTopicForm) · `Main` · `HapjesHeld` · `Profile` · `Children` · `ChildForm` · `Memories` · `AllergenenChildren` · `EersteHapjes` · `DoseForm` · `SymptomLog` · `SymptomForm` · `Learnings` · `LearningDetail` · `LearningPdf` · `Aanraders` · `AanraderProduct`.
...
| `recipes.ts` | `getRecipes`, `getRecipe`, `getRecipesByIds`, `dbToRecipe` mapper (incl. `min_age_months → minAgeMonths` voor de family-layer). Cache per-recipe + lijst. **Lijstquery versmald (2026-09-09)**: `getRecipes()` selecteert `LIST_COLUMNS` in plaats van `*` — `ingredients` en `preparation` zitten er bewust NIET in, want geen enkel lijstscherm toont ze. **VALKUIL**: daarom vult `getRecipes()` de per-recept-cache NIET meer. Zou dat wel gebeuren, dan kreeg het detailscherm of de boodschappenlijst een recept zonder ingrediënten uit de cache. |
...
| `hapjesheld.ts` | `RAG_API_URL` (`EXPO_PUBLIC_RAG_API_URL` override). `getAuthToken`, `authedFetch`, `parseOrThrow<T>`. Endpoints: `getProfile`, `listConversations`, `getConversation`, `deleteConversation`, `renameConversation(id, title)`, `sendChatMessage(req, onText?)`. **Streaming (2026-09-26)**: `sendChatMessage` stuurt altijd `stream: true` en gebruikt **`expo/fetch`** (de RN-fetch kan een body niet stukje per stukje lezen; `expo/fetch` zit in het `expo`-pakket, geen extra dependency). Bij `text/event-stream` leest `readChatStream` via `sseReader` de events `delta`/`done`/`error` (spiegel van de web `js/chat.js`); onbekende events worden genegeerd. De voorgestelde vervolgvragen (`suggestions`-event) zijn op 2026-09-26 op web én app weer geschrapt — inhoudelijk te zwak. Cache-hits, fallback en 429 blijven JSON. **Feedback**: `getChatFeedback(conversationId)` (stil `{}` bij fout) + `sendChatFeedback(messageId, 1\|-1\|0, reden?)` → `/api/chat-feedback` (0 = DELETE). **`recipeIdFromUrl(url)`** herkent `https://community-web.prilleven.be/#/recipe/<id>` (spiegel van `RECIPE_URL_PREFIX` in `api/chat.mjs`, bij een domeinwijziging mee aanpassen). Types: `ChatRequest/Response`, `MonthlyUsage`, `DailyImageUsage`, `ProfileResponse`, `ConversationSummary`, `StoredMessage`, `ConversationDetail`, `ChatError`. |
...
| `children.ts` | **Gecached (2026-09-18)**: `getChildren()` achter de 30 s-cache (`children:all`), gewist door `createChild`/`updateChild`/`archiveChild`. Stond daarvoor op elke schermopening een volledige API-call. Eigen `authedFetch` + `jsonOrThrow`. Types `Child`, `ChildInput`. `BIRTHDATE_REGEX = /^\d{4}-\d{2}-\d{2}$/`. `KNOWN_ALLERGEN_OPTIONS` (9 hoofdallergenen: kippen-ei, pinda, noten, sesam, vis, schaaldieren, soja, tarwe, koemelk — keys identiek aan `js/content/eersteHapjes-allergen-flow.js`). Endpoints: `getChildren()` (GET `/api/children`, server vult `introduced_allergens`-summary), `createChild(input)` (POST), `updateChild(id, patch)` (PATCH `{ id, ...patch }`), `archiveChild(id)` (DELETE `{ id }` → soft delete via `archived_at`). Helpers: `ageInMonths(birthdate)`, `formatAge(birthdate)` → "3 maanden" / "1 jaar 2 maanden". **NB**: `texture_preference` en `has_eczema` zijn op de website verwijderd uit UI sinds v2.2.1 — de DB-kolommen bestaan nog maar de mobile-app stuurt/toont ze niet meer. |
...
| `eersteHapjes.ts` | **Overzichtscache (2026-09-20)**: `getEhOverview(childId)` zit achter een cache van 30 s (`eh:overview:<childId>`); zeven mutaties wissen hem (dose aanmaken/wijzigen/verwijderen, symptoom idem, `patchEhState`). Het allergenenscherm vult hem, het dose- en symptoomformulier lezen eruit — die haalden anders dezelfde gegevens opnieuw op bij de APARTE `/doses`- en `/state`-functions, die sinds de bundeling bijna nooit meer geraakt worden en dus koud staan. Eigen `authedFetch` + `jsonOrThrow` + `okOrThrow`. **Allergen-constants**: `ALLERGEN_FLOW` (9 hoofdallergenen met `key`/`label`/`icon`/`order`/`introFromMonths` (4) / `introBeforeMonths` (12) / `suggestion`), `ALLERGEN_COOLDOWN_DAYS = 2`, `ALLERGEN_TARGET_DOSES = 3`, `REACTION_LEVELS` (geen/mild/ernstig met `counts`/`pauses`/`escalate` flags). **Symptom-constants** (v2.6.0): `SYMPTOM_TYPES` (16 items met `key`/`label`/`icon`/`redFlagSeverity[]` — mirror van server `RED_FLAG_SEVERITIES`; `ademhaling` & `lethargie` triggeren al bij `mild`, `braken`/`koorts`/`zwelling`/`hoesten`/`gewicht` vanaf `matig`, de rest enkel bij `heftig`), `SYMPTOM_SEVERITIES` (mild/matig/heftig met description). Types: `EhState`, `EhDose`, `EhDoseInput`, `AllergenStatus` ('veilig' \| 'allergisch' \| 'paused' \| 'in-progress' \| 'wacht' \| 'locked-age' \| 'excluded' — v2.9.0), `AllergenContext`, `AllergenStateData`, `EhSymptom`, `EhSymptomInput`, `SymptomType`, `SymptomSeverity`, `TimeAfterEating`, `SymptomDuration`, `SymptomWorsened`, `SymptomBehavior`. State-endpoints: `getEhState(childId)`, `patchEhState(childId, patch)` (PATCH met deep-merge op `allergen_state`). Dose-endpoints: `getEhDoses`, `createEhDose` (409 op duplicate `(child, allergen, dose_number)`), `updateEhDose`, `deleteEhDose`. **Symptom-endpoints** (v2.6.0): `getEhSymptoms(childId, { since?, limit? })` (GET `/api/eerste-hapjes/symptoms`), `createEhSymptom(input)` (POST, server vult `red_flag` o.b.v. `RED_FLAG_SEVERITIES`), `updateEhSymptom(id, patch)` (PATCH), `deleteEhSymptom(id)` (DELETE → 204). Helpers: `buildAllergenContext`, `getAllergenStatus`, `successfulDoseCount`, `nextDoseNumber`, `todayIsoDate`, `isRedFlag(type, severity)` (lokale mirror voor live-banner in form), `symptomTypeInfo(type)` (label/icon lookup). Spiegelt `js/content/eersteHapjes-allergen-flow.js` + `js/eersteHapjesStateApi.js` + `api/_lib/eersteHapjes-logs.mjs` op de website. **v2.6.0** dekt state + doses + symptomen; **v2.8.x** voegt setup-flow + welcome-card + pause-hide toe; **v2.9.x** voegt arts-toezicht/exclude-toggle toe (`excluded_keys` patch via `patchEhState`) + volledige 3-staps pause-wizard. Feature-compleet t.o.v. web. |
| `community.ts` (v2.10.0) | **Voorladen (2026-09-18)**: `FEED_PAGE_SIZE`, `prefetchFeed(limit)` en `takePrefetchedFeed(limit)` — de landing laadt de eerste feedpagina eenmalig voor, de tijdlijn verbruikt die belofte bij zijn eerste load (vervalt na 30 s). Bewust géén gewone cache: likes en reacties wijzigen de tellers. Eigen `authedFetch` + `jsonOrThrow` (kopie van `communityProfile.ts`-helpers). Spiegelt `api/community.mjs`. Types `CommunityPost` (`id`, `user_id`, `body`, `category`, `image_path`, `is_pinned`, `edited_at`, `created_at`, `nickname`, `likes_count`, `replies_count`, `has_poll`, `source_type` `'community'\|'chatroom'`, `liked_by_me`, `image_url`/`avatar_url` signed 1u TTL, `poll`, `author_is_admin`, + optioneel voor chatroom-items `title`/`source_room_title`/`source_room_slug`/`last_reply_at` — v2.15.0), `CommunityReply`, `CategoryInfo`. Constants: `POST_CATEGORIES` (algemeen💬, vraag❓, tip💡, mijlpaal⭐, voeding🥕, slapen😴), `POST_BODY_MAX = 4000`, `REPLY_BODY_MAX = 2000`, helper `categoryInfo(key)`. Endpoints: `listPosts({ before?, limit?, category? })` (GET `/api/community/posts`, **cursor-paginatie** via `before` = created_at), `createPost({ body, category })` (POST, 412 zonder nickname), `togglePostLike(postId)` (POST `/like` → `{ liked, count }`), `toggleReplyLike(replyId)` (POST `/replies/:id/like` → `{ liked, count }`, v2.12.1), `listReplies(postId)`, `createReply(postId, body)` (412 zonder nickname), `editPost(postId, body)` (PATCH, owner-check 403, geen edit-window), `deletePost(postId)` (DELETE → `{ ok }`), `editReply(replyId, body)` (PATCH), `deleteReply(replyId)` (DELETE) — v2.12.0, `getIsAdmin(email)` (GET `/api/subscription-status?email=` → `{ is_admin }`, stille false bij fout), **`getAppBadges(since)` → `{ timeline: number }`** (v3.1.x, GET `/api/community/app-badges?since=<iso>`, server-side tijdlijn-badge-teller: nieuwe posts+replies+gevolgde topics, admin-logica + 6-weken-vervaltermijn server-side; via barrel geëxporteerd, gebruikt door `services/notifications.ts::fetchTimelineBadge`). **Moderatie — App Store 1.2 (v3.0.1)**: `reportCommunityTarget(targetType: 'post'\|'reply', targetId, reason?)` (POST `/api/community/report`), `BlockedUser` (`blocked_id`, `nickname`, `created_at`), `listBlocks()` (GET `/api/community/blocks`), `blockUser(blockedId)` (POST `/api/community/blocks`), `unblockUser(blockedId)` (DELETE `/api/community/blocks/:id`). Één-richting-blok: de server filtert de geblokkeerde auteur uit ALLE feed-leesopvragingen. **NB**: `deleteReply` wordt NIET via de barrel geëxporteerd (botst met `chatRooms.deleteReply`); `TimelineScreen` importeert die rechtstreeks uit `services/community`. `reportCommunityTarget`/`listBlocks`/`blockUser`/`unblockUser`/`BlockedUser` hebben unieke namen → wél via de barrel. |
| `subscription.ts` (v3.2.0) | Toegangscontrole op het lidmaatschap. Spiegelt `fetchSubscriptionStatus` + `subscriptionAccessMessage` uit de website (`js/supabase.js`). `getSubscriptionStatus(email)` → `SubscriptionStatus { active, reason: 'not_registered'|'cancelled'|'expired'|null, end_date, is_admin }` via **GET `/api/subscription-status?email=`**. Bewust ZONDER Authorization-header — het endpoint is publiek, waardoor de check blijft werken terwijl een JWT ververst wordt en een tokenprobleem nooit iemand buitensluit. **FAIL-OPEN**: netwerkfout, timeout (10s `AbortController`) of onleesbaar antwoord → `active: true`. Alleen een expliciete `active: false` van de server blokkeert. `accessMessage(status)` geeft de NL-uitlegzin per `reason`; `formatEndDate(iso)` → "12 september 2026". Gebruikt door `lib/useSubscriptionGate.ts` en `ProfileScreen`. **Opzegverzoek (v3.2.0)**: `getOpzegverzoek()` (GET `/api/opzegverzoek` → `{ aangevraagd_op } | null`, stil `null` bij elke fout — mag het profiel niet blokkeren) en `createOpzegverzoek()` (POST → `{ aangevraagd_op, al_ingediend }`, gooit WEL met NL-melding want anders denkt iemand opgezegd te hebben terwijl er niets vastligt). Deze twee gaan **wel** met JWT; de server haalt het e-mailadres uit het token, nooit uit de body. Server is idempotent: een tweede indiening geeft geen tweede rij en geen tweede mail. Zelf opzeggen in Plug&Pay kan niet (Ultimate-only, wij draaien Premium) → handmatige afhandeling. |
| `notifications.ts` (**herschreven 2026-09-09**) | Eén functie: `fetchAppBadgeCounts(since)` → `{ timeline, chatrooms: { total, perRoom, perTopic } }`, een dunne wrapper rond `getAppBadges`. **Beide** tellers komen nu server-side uit **één** verzoek. Vóór deze wijziging telde de app de chatruimtes zelf: per ronde `listRooms()` + `getRoom()` per ruimte + `getTopic()` per recent actief topic — vijf tot twintig verzoeken per telling, elke minuut. `scanChatroomActivity`, `countNewAdminChatroomActivity`, `withExpiry` en `isNewerThan` zijn daarmee verdwenen (184 → 62 regels). De admin-telregel, de 6-weken-vervaltermijn, de geblokkeerde auteurs én de per-topic markeerpunten zitten server-side; die laatste leest de server zelf uit `user_badge_state`, dezelfde bron als de push-badge, dus de app stuurt ze niet mee. Defensief: bij een fout nullen. |
| `chatRooms.ts` (v2.11.0) | Eigen `authedFetch` + `jsonOrThrow` + `okOrThrow` (kopie van `community.ts`-helpers). Spiegelt `api/chat-rooms.mjs`. **Constants**: `ROOMS` (4 vaste rooms: melk-voeding🍼, eerste-hapjes🥄, allergieen-overgevoeligheden🌾, feedback💬), `TOPIC_TITLE_MAX = 120`, `TOPIC_BODY_MAX = 4000`, `REPLY_BODY_MAX = 2000`, `EDIT_WINDOW_MS = 15 min`. Types: `ChatRoom`, `ChatRoomDetail` (+ `admin_intro`), `ChatTopic` (`title`, `body`, `is_pinned`, `replies_count`, `last_reply_at`, `author_is_admin`, nickname/avatar_url signed), `ChatReply`, `AdminIntro`. Endpoints: `listRooms()` (GET `/api/chat-rooms`), `getRoom(slug)` (GET `/api/chat-rooms/:slug` → `{ room, topics }`), `getTopic(id)` (GET `/api/chat-rooms/topics/:id` → `{ topic, replies }`), `createTopic(slug, { title, body })` (POST, 412 zonder nickname / 422 moderatie), `updateTopic(id, patch)` (PATCH, 403 buiten eigenaar/venster), `deleteTopic(id)` (DELETE → `{ ok: true }`), `createReply(topicId, body)` / `updateReply(id, body)` / `deleteReply(id)`. **Follow/unread (v2.14.0)**: `getUnread()` (GET `/api/chat-rooms/unread` → `UnreadCounts` `{ rooms: {room_id:count}, topics: {topic_id:count} }`, defensief → lege maps bij fout), `followRoom(slug)` (POST `/api/chat-rooms/:slug/follow`), `unfollowRoom(slug)` (DELETE), `markRoomRead(slug)` (POST `/api/chat-rooms/:slug/read`, stil falen — mag het openen van de room niet blokkeren). `ChatRoom` + `ChatTopic` hebben `is_followed?: boolean`. Helpers: `getCurrentUserId()` (uit `supabase.auth.getSession()` — UserContext heeft enkel de e-mail, niet de UUID, nodig voor eigenaarschap-checks), `isWithinEditWindow(createdAt)`, `roomEmoji(slug)`. **NB**: `createReply` + `REPLY_BODY_MAX` worden NIET via de barrel geëxporteerd (botsen met `community.ts`); `ChatTopicScreen` importeert ze rechtstreeks uit `services/chatRooms`. **Topic-follow** (`/api/chat-rooms/topics/:id/follow` + `/read`) bestaat server-side maar is nog niet in de mobile-app gewired (enkel room-follow). **Moderatie — App Store 1.2 (v3.0.1)**: `reportChatTarget(targetType: 'topic'\|'reply', targetId, reason?)` (POST `/api/chat-rooms/report`, via `okOrThrow`). Blokkeren hergebruikt `blockUser` uit `services/community` (één gedeelde `user_blocks`-tabel); de server filtert geblokkeerde auteurs ook uit room/topic-leesopvragingen. |

| `aanraders.ts` (v3.2.0) | De affiliatepagina, **Supabase-direct** (patroon van `recipes.ts`, niet van `hapjesheld.ts`): de drie `affiliate_*`-tabellen zijn publiek leesbaar voor anon waar `zichtbaar = true`, dus er is geen Vercel-endpoint nodig. `getAanraders()` haalt categorieën, producten en downloads in één keer op (30 s cache, key `aanraders:all`) en plakt `categorie_slug`/`categorie_titel` op elk product. Types `AanraderCategorie` / `AanraderProduct` / `AanraderDownload` / `RelatieType`. Constants letterlijk uit `api/aanraders.mjs`: `RELATIE_LABELS`, `LABEL_TEKST`, `INTRO_TEKST`. Helpers: `leeftijdLabel`, `zoekTekst` (spiegel van `data-zoek`), `veiligeUrl` (enkel http(s), spiegel van `safeUrl`). **`magKoopknopTonen(p)`**: false op iOS wanneer de affiliate-link naar `plugandpay.com` wijst — dat zijn Pril Levens eigen digitale producten (masterclass, roadmap) en App Store 3.1.1 verbiedt een call-to-action naar een andere aankoopmethode voor digitale content. De kaart blijft staan, enkel de knop verdwijnt — en sinds 2026-09-18 ook de zin "Te bekijken in de webversie", die zei waar je het koopt. Affiliatelinks van derden (fysieke producten, externe diensten) vallen daar niet onder. Beheren blijft admin-only op de website. |
| `learnings.ts` (v2.17.0) | **Gecached (2026-09-18)**: `getLearnings()` achter de 30 s-cache per querystring, gewist door `toggleLearningFavorite` en `putLearningBookmark` (die bepalen het hartje en de badges Bezig/Afgerond). `{ force: true }` omzeilt hem voor pull-to-refresh. Eigen `authedFetch` + `jsonOrThrow` (kopie van `memory.ts`-helpers, `RAG_API_URL` uit `./hapjesheld`). Read-only viewer van de website-bibliotheek; aanmaken/bewerken/verwijderen blijft admin-only op de website. Types: `LearningKind` (`pdf`/`blog`/`video`), `Learning` (`id`, `kind`, `title`, `description`, `thumbnail_url`, `duration_sec`, `tags`, `created_at`, `is_favorite`), `LearningDetail extends Learning` (+ `body_html` voor blog, `signed_url` voor pdf/video). Endpoints: `getLearnings({ kind?, favoritesOnly? })` (GET `/api/learnings`, `?kind=`/`?favorites=1`), `getLearning(id)` (GET `/api/learnings/:id` → detail incl. `signed_url` (pdf 10min / video 4u TTL) of `body_html`), `toggleLearningFavorite(id)` (POST `/api/learnings/:id/favorite` → nieuwe `is_favorite`). Helpers: `learningKindIcon` (📄/🎬/📝), `learningKindLabel` (Document/Video/Blog), `formatDuration(sec)` ("m:ss"). **Bladwijzer-sync (v2.18.0)**: type `LearningBookmarkPosition` (`page_nr?` PDF / `scroll_px?` blog / `seconds?` video). `getLearningBookmark(id)` (GET `/api/learnings/:id/bookmark` → `position | null`), `putLearningBookmark(id, position)` (PUT `{ position }`). Gedeeld met de website (`user_learning_bookmarks`), dus bladwijzer/positie synchroniseren tussen web en mobiel (pdf-pagina + video-seconden gewired in de app; blog `scroll_px` nog niet). Spiegelt `api/learnings.mjs` op de website (notities/clips bewust buiten scope). |
| `push.ts` (v3.2.0 — app-icoon-push) | Expo-push-notificaties + app-icoon-badge. `expo-notifications` + `expo-device` + `expo-constants`. `setNotificationHandler` (foreground: banner+list+sound+badge). `registerPushToken()`: vraagt toestemming, haalt Expo-push-token (`getExpoPushTokenAsync({ projectId })`), POST `/api/community/push/register { token, platform }`. Enkel op echt toestel (`Device.isDevice`), gooit nooit. `deregisterPushToken()`: DELETE `/api/community/push/register { token }` — VÓÓR `signOut` (JWT nog geldig). `syncBadgeState(patch: BadgeStatePatch)`: PUT `/api/community/badge-state` — spiegelt `timeline_seen_at`/`chatrooms_seen_at`/`topic_reads` naar de server zodat die bij een push (app dicht) het absolute app-icoon-getal per ontvanger kan berekenen (`computeTotalBadge` in `api/_lib/badges.mjs`). `setAppBadge(n)` / `clearAppBadge()` (`Notifications.setBadgeCountAsync`). Alle functies defensief (gooien nooit). Aangeroepen vanuit `NotificationContext` (register bij login, `syncBadgeState` bij init + `markTimelineSeen`/`markTopicSeen`, `setAppBadge(tl+chatroom.total)` bij elke telling) en `UserContext.logout` (`deregisterPushToken` + `clearAppBadge`). **NB remote push werkt NIET in Expo Go — enkel in een EAS dev/preview/production-build.** Server-zijde: `api/_lib/push.mjs` (Expo Push API `https://exp.host/--/api/v2/push/send`, tokens in `push_tokens`, recipients + `notifyNewActivity` bij nieuwe post/reply/topic). **Tik-afhandeling (v3.2.0)**: `addPushResponseListener(cb)` (tik terwijl de app draait) + `getInitialPushResponse()` (koude start, `getLastNotificationResponseAsync`) leveren `PushResponse { data: PushData, id }`. `PushData` = `{ kind?: 'timeline_post'|'timeline_reply'|'chatroom_topic'|'chatroom_reply', postId?, topicId?, roomSlug?, roomTitle? }` — de `data`-payload van de server. De service navigeert zelf NIET; dat doet `navigation/pushRouting.tsx`. |
...
colors = {
  ...
  greenText: '#4F7D6C', greenDark: '#3F6558',   // HET merkgroen (v3.2.0)
  secondary: '#98C3A4', secondaryDark: '#82BE93',   // AFGESCHREVEN, niet gebruiken
...
- **Groen = `colors.greenText` (#4F7D6C), voor tekst én vlakken.** Sinds v3.2.0 (web sinds 2026-08-01) is dat de enige groentint: koppen, knoppen (`btnSecondary`, genereerknop), badges, chatbubbels, avatars, volgknoppen, de leeftijd-badge bij recepten, en alles groens in tijdlijn en chatruimtes. `colors.greenDark` (#3F6558) is de donkerdere tint — voor een ingedrukte staat, of wanneer er écht twee groenstappen naast elkaar nodig zijn (`AVATAR_COLORS` in `lib/familyLayer.ts` en de importance-schaal in `MemoriesScreen`, allebei 1-op-1 gespiegeld van `_AVATAR_COLORS` in de web `js/utils.js`). Lichte groenvlakken zijn `rgba(79, 125, 108, α)`. De oude salie-vulgroenen `colors.secondary(-Dark)` (#98C3A4 / #82BE93) staan nog in `theme.ts` maar worden nergens meer gebruikt — pak ze niet voor nieuw werk. Terracotta `colors.primary` blijft de hoofdkleur.
- **Geen automatische standaard-icoontjes/emoji in het design.** Gebruik in de UI géén default emoji-iconen (bv. `opt.icon` van allergenen) tenzij de gebruiker er expliciet om vraagt. Toon labels zonder emoji. De `icon`-velden in constants mogen blijven bestaan voor andere doeleinden, maar render ze niet zomaar.
...
| `prilleven_learning_progress_<email>` | `lib/learningProgress.ts` | JSON `Record<learningId, { completed_at }>` — "Afgerond" in Mijn leertraject (lokale preview, v3.2.0) |
| `notif_seen_timeline_<email>` | `NotificationContext` | ISO-tijdstip "tijdlijn laatst gezien" |
| `notif_seen_chatrooms_<email>` | `NotificationContext` | ISO-tijdstip "chatruimtes laatst gezien" (baseline voor per-topic) |
| `notif_topic_reads_<email>` | `NotificationContext` | JSON `Record<topicId, ISO>` — per-topic "gezien"-tijdstip (v3.1.0) |
...
| `…/api/chat` | RAG-chat (met `stream: true` als text/event-stream) | `services/hapjesheld.ts::sendChatMessage` |
| `…/api/chat-feedback` GET/POST/DELETE | Duim omhoog/omlaag per antwoord (+ reden bij omlaag) | `services/hapjesheld.ts::getChatFeedback` / `sendChatFeedback` |
...
| `…/api/community/app-badges?since=` GET | Server-side badge-tellers → `{ timeline, chatrooms: { total, perRoom, perTopic } }`. Chatruimte-baseline + per-topic markeerpunten leest de server zelf uit `user_badge_state` | `services/community.ts::getAppBadges` |
...
| `…/api/subscription-status?email=` GET | Toegangscontrole lidmaatschap (`active`/`reason`/`end_date`), publiek endpoint | `services/subscription.ts::getSubscriptionStatus` |
| `…/api/opzegverzoek` GET/POST | Openstaand opzegverzoek opvragen / indienen (rij in `cancellation_requests` + mail via Resend naar hallo@prilleven.be) | `services/subscription.ts::getOpzegverzoek` / `createOpzegverzoek` |
| `…/api/community/report` POST | Tijdlijn-post/reply rapporteren (App Store 1.2) | `services/community.ts::reportCommunityTarget` |
| `…/api/community/blocks` GET/POST | Geblokkeerde gebruikers lijst + iemand blokkeren (één-richting) | `services/community.ts::listBlocks` / `blockUser` |
| `…/api/community/blocks/:id` DELETE | Gebruiker deblokkeren | `services/community.ts::unblockUser` |
| `…/api/community/push/register` POST/DELETE | Expo-push-token opslaan/wissen (app-icoon-push, v3.2.0) | `services/push.ts::registerPushToken` / `deregisterPushToken` |
| `…/api/community/badge-state` PUT | Server-spiegel "laatst gezien" (`timeline_seen_at`/`chatrooms_seen_at`/`topic_reads`) voor de push-badge | `services/push.ts::syncBadgeState` |
| `…/api/community/app-badges?since=` GET | (zelfde route, zie hierboven) | `getAppBadges` → `notifications.ts::fetchAppBadgeCounts` |
| `…/api/chat-rooms/report` POST | Chat-topic/reply rapporteren (App Store 1.2) | `services/chatRooms.ts::reportChatTarget` |
...
| `…/api/eerste-hapjes/state?include=doses,symptoms` GET | **Gecombineerd** → `{ state, doses, symptoms }`. Eén verzoek voor het hele allergenenscherm i.p.v. drie naar drie aparte functions (koude starts, zie §11) | `services/eersteHapjes.ts::getEhOverview` |
...
| `supabase.from('affiliate_categories'/'affiliate_products'/'affiliate_downloads')` | Aanraders (publiek leesbaar via RLS) | `services/aanraders.ts::getAanraders` |
...
**Plan-tier:** de website draait sinds 2026-07-29 op **Vercel Pro** en **Supabase Pro**. De oude Hobby-limiet van 12 functions per deployment geldt niet meer (er staan er 21), dus een nieuw endpoint hoeft niet per se in een bestaande catch-all gepropt te worden. **Wat wél telt: koude starts.** Elke function koelt apart af. Drie parallelle calls naar drie functions betekent drie koude starts — gemeten 3,8 s voor `/eerste-hapjes/doses` alleen. Bundel daarom aanroepen die altijd samen gebeuren in één endpoint (zie `getEhOverview`), zodat er één function is die bovendien vaker geraakt wordt en dus warm blijft.

---

## 12. Bekende valkuilen

- **`getRecipes()` levert onvolledige recepten.** Sinds 2026-09-09 zonder `ingredients` en `preparation`, en het vult de per-recept-cache niet meer. Heb je een volledig recept nodig, gebruik `getRecipe(id)` of `getRecipesByIds`.
...
**Expo Go werkt NIET meer** (sinds 2026-09-09): de App Store levert enkel de nieuwste Expo Go (SDK 57) en dit project staat op SDK 54; een oudere versie bijzetten kan niet op iOS. Testen op een toestel gaat via een EAS **development build** (cloud-build, dus geen Xcode nodig — dat staat ook niet op de machine):

```bash
eas device:create                                  # iPhone registreren (eenmalig)
eas build --platform ios --profile development
npx expo start --dev-client -c                     # of het launch-profiel mobile-dev
```

Push werkt niet in een dev-build: `aps-environment` staat hard op `production` terwijl een dev-build de APNs-sandbox gebruikt. Voor pushtests een `preview`-build maken.
...
5. 🟡 **Chatruimtes** — categorische rooms (Melk & voeding, Eerste hapjes, Allergieën & overgevoeligheden, Feedback) + topics.
   ...
   - ✅ v2.15.0: gevolgde topics zichtbaar op de tijdlijn — de server merget gevolgde chatroom-topics al in `GET /api/community/posts` (`source_type==='chatroom'`); `TimelineScreen` filtert ze niet meer weg maar rendert ze als `ChatroomTopicCard` (bron-badge + titel + snippet + "Open discussie"), tik → cross-tab naar `ChatTopic`. Tijdlijn-notificatieteller telt admin-posts + alle nieuwe gevolgde chatroom-items op (in v2.15.0 nog client-side `countNewAdminTimelinePosts`; sinds v3.1.x server-side via `fetchTimelineBadge` → `GET /api/community/app-badges`, incl. replies — zie §14 punt 6).
...
6. 🟡 **Notificaties** — admin-post badges op de footer-tabs.
   ...
   - ✅ v3.1.0: chatruimte-badges effectief tonen, tot op topic-niveau, met vervaltermijn:
     - **Replies meegeteld** — `countNewAdminChatroomPosts` vervangen door `countNewAdminChatroomActivity(since, includeAllAuthors?, topicReads?) → ChatroomActivity { total, perRoom, perTopic }`: telt per topic het nieuwe topic ÉN nieuwe replies (replies enkel gescand voor topics met `last_reply_at > effSince`).
     - **Per-room badge op admin-activiteit** — locatie 3 (`ChatRoomsListScreen`) gebruikt `chatroomRoomCounts[room.id]` uit `NotificationContext` i.p.v. de follow-gebaseerde `getUnread()`. Gevolgd-stip (`is_followed`) blijft los hiervan getoond.
     - **Per-topic badge (locatie 4)** — `ChatRoomScreen` toont per topic-card een rode badge (`chatroomTopicCounts[topic.id]`). Het openen van een room wist NIET meer alles (`markChatroomsSeen` verwijderd; footer-tab + `ChatRoomScreen` roepen `refresh()` aan); een topic-badge wist pas wanneer je dát topic opent (`ChatTopicScreen` → `markTopicSeen(topicId)`). Per-topic markeerpunten in `notif_topic_reads_<email>`; effectief referentiepunt = laatste van globale baseline + per-topic "gezien". Footer-/room-tellers krimpen navenant terwijl ongelezen topics zichtbaar blijven.
     - **Vervaltermijn 6 weken** — `withExpiry()` in `services/notifications.ts` zet een ondergrens (`BADGE_MAX_AGE_MS`) op de baseline van de chatruimte-teller; de tijdlijn-teller draagt diezelfde 6-weken-ondergrens sinds v3.1.x server-side. Items ouder dan 6 weken tellen nooit nog als nieuw, ook niet als de tab/het topic nooit geopend werd → badges lopen niet eindeloos op.
   - ✅ v3.1.x: tijdlijn-badge telt nu ook **replies** — server-side. `countNewAdminTimelinePosts` (client-side, telde enkel posts want `community_posts` heeft geen `last_reply_at`) vervangen door `fetchTimelineBadge(since)` → `getAppBadges(since)` → **`GET /api/community/app-badges?since=<iso>`**. De server telt nieuwe posts ÉN replies in de community-feed + nieuwe gevolgde chatroom-topics, bepaalt zelf de admin-status (client stuurt geen `includeAllAuthors` meer voor de tijdlijn), en past de telregel + 6-weken-vervaltermijn + geblokkeerde-auteurs server-side toe. Endpoint is een route in de bestaande `api/community.mjs` catch-all; telfunctie `countTimelineBadge` staat lokaal in dat bestand omdat het `loadFollowedChatroomTopics` hergebruikt. Website-werk op branch `tijdlijn-badge-reacties`.
   - ✅ v3.2.0: **app-icoon-push (Expo Push, server-side trigger)**. De rode badge op het app-icoon (home-screen, ook wanneer de app dicht is) = ALLES opgeteld (tijdlijn + chatruimtes), zelfde admin/gevolgd-telregel. Werking:
     - **App** (`services/push.ts` + `NotificationContext`): bij (her)login `registerPushToken()` → Expo-push-token → POST `/api/community/push/register`. Bij uitloggen `deregisterPushToken()` (vóór signOut). `syncBadgeState()` spiegelt de "laatst gezien"-tijdstippen (`timeline_seen_at`/`chatrooms_seen_at`/`topic_reads`) naar de server (bij init + `markTimelineSeen`/`markTopicSeen`) zodat de server het absolute getal kan berekenen. Zolang de app open is blijft `setAppBadge(tl+chatroom.total)` het lokaal spiegelen.
     - **Website** (`api/_lib/push.mjs` + `api/_lib/badges.mjs`): `notifyNewActivity(kind, ctx, notif)` wordt getriggerd bij nieuwe tijdlijn-post/reply (`api/community.mjs`) en nieuw chatroom-topic/reply (`api/chat-rooms.mjs`). Bepaalt de ontvangers (timeline_post→iedereen met token; timeline_reply→admins tenzij auteur admin; chatroom_topic/reply→admins + room-/topic-volgers tenzij auteur admin; steeds minus auteur + minus wie de auteur blokkeerde), berekent per ontvanger `computeTotalBadge(userId)` (leest `user_badge_state`, telt tijdlijn+chatruimtes met dezelfde 6-weken-vervaltermijn), en stuurt één Expo-push per token (`https://exp.host/--/api/v2/push/send`, chunks van 100) met `{ title, body, badge, sound, priority:'high' }`. Volledig defensief: een push-fout breekt nooit de post/reply-request.
     - **DB**: migratie `supabase-migrations/2026-07-08-push-notifications.sql` → `push_tokens` (token PK, user_id, platform) + `user_badge_state` (user_id PK, timeline_seen_at, chatrooms_seen_at, topic_reads jsonb), beide RLS owner-only.
     - **Endpoints** (routes in de bestaande `api/community.mjs` catch-all): `POST/DELETE /api/community/push/register`, `PUT /api/community/badge-state`.
     - ✅ v3.2.0: `app.json`-config rond (`ios.entitlements.aps-environment: production`, expo-notifications-plugin, `android.googleServicesFile: ./google-services.json`), Android-FCM-credentials geüpload (Firebase-project `prilleven-d2186`), en **tik-afhandeling**: `navigation/pushRouting.tsx` stuurt een tik naar de Tijdlijn of rechtstreeks naar het juiste ChatTopic, op basis van de `data`-payload die de server sinds 2026-09-02 meestuurt.
     - ⬜ Nog nodig vóór productie: APNs-sleutel aanmaken (`eas credentials --platform ios`) en de eerste build. Het **monochrome notificatie-icoon** is er sinds 2026-09-20: `assets/notification-icon.png` (96×96, wit takje op transparant). Android gebruikt enkel de transparantie van dat bestand en negeert kleur — een gevuld kleurenlogo wordt daar een witte vlek. De accentkleur komt uit de `color`-optie van de expo-notifications-plugin. De **Apple Program License Agreement** is op 2026-09-09 geaccepteerd en blokkeert niets meer. `aps-environment` staat hard op `production`: klopt voor preview/production, maar bij een `development`-build is dat het eerste om te controleren als er geen token komt.
     - ⬜ Android push is **ongetest**: er is geen Android-toestel beschikbaar. De preview-build van 2026-09-09 bewijst enkel dat de configuratie compileert.
...
8. ✅ **Aanraders** — de affiliatepagina, native in v3.2.0. Leest de drie `affiliate_*`-tabellen
   Supabase-direct (anon mag lezen waar `zichtbaar = true`), dus geen website-endpoint. Overzicht
   met zoeken + filters en een productdetail; géén categoriescherm (dat bestaat op de web vooral
   voor Google). Beheren blijft admin-only op de website.
   - ⬜ Open: niets. Velden als `lange_beschrijving`, `nadelen` en `faq` staan nog leeg in de DB;
     de app toont ze zodra ze gevuld worden.

9. ✅ **Toegangscontrole & lidmaatschap** — v3.2.0. Vóór deze versie controleerde de app NERGENS of
   het abonnement nog liep: `allowed_users` werd enkel bij registratie geraadpleegd, dus een lid met
   een verlopen abonnement hield onbeperkt toegang. `lib/useSubscriptionGate.ts` checkt nu bij login,
   elke 60 s in de voorgrond en bij elke `AppState 'active'`. Plus het opzegverzoek in het profiel.
   - Op iOS bewust géén link naar de checkout (App Store 3.1.1) — zie `constants/links.ts`.

10. 🟡 **Gamification** — fase C uit `PLAN-TIMELINE.md`.
   ...
   - ✅ De HapjesHeld-"Dit helpt mij" is vervangen door duim omhoog/omlaag via `/api/chat-feedback` (server-side, dus wel gedeeld tussen web en app — 2026-09-26).
