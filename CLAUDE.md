...
| iOS bundle | `be.prilleven.mobileapp`, ascAppId `6762270908`, buildNumber `65` (v3.1.0) |
| Android pkg | `be.prilleven.mobileapp`, versionCode `70` (v3.1.0) |
...
| `ProfileScreen.tsx` | 6 secties: **Account** (e-mail + uitloggen), **Community** (nickname-input met regex-validatie + Opslaan-knop, avatar-blok met `AvatarButton`-preview + "Foto kiezen/wijzigen/Verwijderen", upload-pipeline: `expo-image-picker` → `expo-image-manipulator` resize 512px JPEG q=0.8 → signed Storage URL → `PUT /api/community/profile { avatar_path }`), **Mijn kinderen** (knop → `ChildrenScreen`), **Dieet in het gezin** (9 chips uit `DIET_OPTIONS`, optimistic toggle met 400ms debounce + saveInFlight-ref + pending-queue, status-pill saving/saved/error, flush-on-unmount; vereist bestaand community-profile anders inline hint), **Voorkeuren & privacy** (HapjesHeld memory-toggle via `Switch`, optimistic + rollback, plus knop "Bekijk opgeslagen geheugen →" naar `MemoriesScreen`), **Mijn gegevens** (GDPR-export via `File`/`Paths` + `Sharing.shareAsync`, GDPR-delete via 2-staps modal met `VERWIJDER`-bevestiging), **Juridisch** (v2.20.0 — Privacybeleid + Gebruiksvoorwaarden, elk `Linking.openURL` naar `PRIVACY_URL`/`TERMS_URL` op `community-web.prilleven.be`), **Geblokkeerde gebruikers** (v3.0.1, App Store 1.2 — lazy `listBlocks()`, rij per geblokkeerde met nickname + Deblokkeren-knop → `unblockUser`; lege lijst is geen blokkerende fout). Header: `ChevronBack` + titel. Lokale `ChevronBack` om require-cycle met RootStack te vermijden. **Account uitgebreid (v3.3.0)**: rij "Lidmaatschap" (`Actief tot <datum>` / `Actief` / `Niet actief` via `getSubscriptionStatus` + `formatEndDate`) plus een **opzegblok** — intro + knop "Abonnement opzeggen" met `Alert`-bevestiging → `createOpzegverzoek()`. Staat er al een verzoek open (`getOpzegverzoek()` bij mount), dan verdwijnt de knop en verschijnt de bevestigingstekst met datum. Die status komt van de server, niet uit AsyncStorage, dus hij klopt ook na herinstalleren of op een ander toestel. `PRIVACY_URL`/`TERMS_URL` komen sinds v3.3.0 uit `constants/links.ts` in plaats van lokale constanten. |
...
| `subscription.ts` (v3.3.0) | Toegangscontrole op het lidmaatschap. Spiegelt `fetchSubscriptionStatus` + `subscriptionAccessMessage` uit de website (`js/supabase.js`). `getSubscriptionStatus(email)` → `SubscriptionStatus { active, reason: 'not_registered'|'cancelled'|'expired'|null, end_date, is_admin }` via **GET `/api/subscription-status?email=`**. Bewust ZONDER Authorization-header — het endpoint is publiek, waardoor de check blijft werken terwijl een JWT ververst wordt en een tokenprobleem nooit iemand buitensluit. **FAIL-OPEN**: netwerkfout, timeout (10s `AbortController`) of onleesbaar antwoord → `active: true`. Alleen een expliciete `active: false` van de server blokkeert. `accessMessage(status)` geeft de NL-uitlegzin per `reason`; `formatEndDate(iso)` → "12 september 2026". Gebruikt door `lib/useSubscriptionGate.ts` en `ProfileScreen`. **Opzegverzoek (v3.3.0)**: `getOpzegverzoek()` (GET `/api/opzegverzoek` → `{ aangevraagd_op } | null`, stil `null` bij elke fout — mag het profiel niet blokkeren) en `createOpzegverzoek()` (POST → `{ aangevraagd_op, al_ingediend }`, gooit WEL met NL-melding want anders denkt iemand opgezegd te hebben terwijl er niets vastligt). Deze twee gaan **wel** met JWT; de server haalt het e-mailadres uit het token, nooit uit de body. Server is idempotent: een tweede indiening geeft geen tweede rij en geen tweede mail. Zelf opzeggen in Plug&Pay kan niet (Ultimate-only, wij draaien Premium) → handmatige afhandeling. |
...
colors = {
  ...
  greenText: '#4F7D6C', greenDark: '#3F6558',   // HET merkgroen (v3.3.0)
...
| `prilleven_learning_progress_<email>` | `lib/learningProgress.ts` | JSON `Record<learningId, { completed_at }>` — "Afgerond" in Mijn leertraject (lokale preview, v3.4.0) |
