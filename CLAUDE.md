...
Lees dit ALTIJD eerst voordat je code wijzigt. Dit document is geschreven op basis van een volledige lezing van de codebase op `v2.6.0` en bijgewerkt t/m `v2.17.1`. Toekomstige Claude-sessies moeten dit bijwerken zodra de waarheid afwijkt.
...
| iOS bundle | `be.prilleven.mobileapp`, ascAppId `6762270908`, buildNumber `55` (v2.17.1) |
| Android pkg | `be.prilleven.mobileapp`, versionCode `59` (v2.17.1) |
...
├── index.ts                         expo entry
├── app.json                         expo config (version 2.17.1, permissions in NL)
...
| `LandingScreen.tsx` | 4 grote `AnimatedTile`-tegels (spring scale 0.96 → 1), **alleen titel — geen beschrijvingstekst** (v2.17.1). Volgorde (v2.17.1): Receptenboek & Weekschema → HapjesHeld 2.0 → Learnings → Allergenen-introductie. + `AvatarButton` rechtsboven die `Profile` opent. `useFocusEffect` refresht `community avatar_url` zodat een upload meteen zichtbaar is. De allergenen-tile navigeert naar `AllergenenChildren` (kind-picker); de Learnings-tile (terracotta overlay, `assets/landing-learnings.png`) naar `Learnings`. |
...
| `LearningsScreen.tsx` | Learnings-bibliotheek (v2.17.0), read-only. Laadt alle items via `getLearnings()` (geen server-filter) met `useFocusEffect` + `RefreshControl`. Client-side `filtered` useMemo: type-filter (KIND_FILTERS chips Alle/Blog/Document/Video) + favorieten-only (hart-chip) + zoeken over titel/beschrijving/tags. `FlatList` van kaarten: thumbnail (`Image` of emoji-placeholder via `learningKindIcon`), type-badge + duur (`formatDuration`), titel, beschrijving, hartje-favorietknop (optimistic toggle met rollback via `toggleLearningFavorite`). **Tap-gedrag (v2.17.1)**: documenten (`kind==='pdf'`) openen meteen in de in-app browser — `onOpen` haalt een verse `getLearning(id)` op en doet `WebBrowser.openBrowserAsync(signed_url)` (spinner-overlay op de thumb via `openingId`, geen tussenscherm); blog/video → `navigate('LearningDetail', { id, title })`. Lokale `ChevronBack` + titel "Learnings". Empty-state met `book-open`-icoon. |
