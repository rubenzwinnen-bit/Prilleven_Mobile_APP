...
6. 🟡 **Notificaties** — admin-post badges op de footer-tabs.
   ...
   - ✅ v3.2.0: **app-icoon-push (Expo Push, server-side trigger)**. De rode badge op het app-icoon (home-screen, ook wanneer de app dicht is) = ALLES opgeteld (tijdlijn + chatruimtes), zelfde admin/gevolgd-telregel. Werking:
     ...
     - ⬜ Nog nodig vóór productie: APNs-sleutel aanmaken (`eas credentials --platform ios`), een monochroom notificatie-icoon, en de eerste build. De **Apple Program License Agreement** is op 2026-09-09 geaccepteerd en blokkeert niets meer. `aps-environment` staat hard op `production`: klopt voor preview/production, maar bij een `development`-build is dat het eerste om te controleren als er geen token komt.
