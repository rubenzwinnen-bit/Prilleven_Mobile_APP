...
| `WeekScheduleScreen.tsx` | 2 sub-tabs (`active` / `generate`), 3 presets (`today` / `today-tomorrow` / `week`). Genereer-knop + per-slot 🔄 refresh. Modal "Opslaan met naam". **Look v3.2.0 (web-pariteit)**: sub-tabs zijn platte teksttabs met een 3px **groene** onderlijn onder de actieve (inactief `darkLight`, niet grijs) — spiegel van `.subtab-btn`. De dagkiezer is een **segmented control** (`presetBar`: één bak met `colors.bg`, 1px `colors.light`-rand, radius 12, padding 4) met daarin transparante pillen; de actieve pil is gevuld met `greenText` + zachte groene schaduw — spiegel van `.day-selector-bar/-btn`, vervangt de drie losse omrande terracotta-knoppen. Naam van het schema + dagkiezer zitten samen in één lage `activeToolbar` zonder kaart of uitleg (`.active-schedule-toolbar`); de titel is daardoor 17px i.p.v. 24px. Dagblokken hebben een rondom lopende `rgba(79,125,108,0.14)`-rand met radius 18 (i.p.v. een dikke linkerbalk); vandaag valt op doordat die rand terracotta wordt. De VANDAAG-badge is groen. De genereer-knop is groen (`#btn-generate`). **Nog open**: de vijf compacte fototegels per dag + het Cooked it-vinkje horen bij de gamification (blok 4). |
...
| `recipes.ts` | `getRecipes`, `getRecipe`, `getRecipesByIds`, `dbToRecipe` mapper (incl. `min_age_months → minAgeMonths` voor de family-layer). Cache per-recipe + lijst. |
...
| `notifications.ts` (v2.13.0, admin-modus v2.13.1, chatroom-feed v2.15.0, replies+per-room v3.1.0, per-topic+vervaltermijn v3.1.0, **tijdlijn server-side v3.1.x**) | Telfuncties voor de notificatie-badges op de footer-tabs (geen push). **`fetchTimelineBadge(since)` → number** (v3.1.x, vervangt `countNewAdminTimelinePosts`): dunne wrapper rond `getAppBadges(since)` uit `services/community.ts` → `GET /api/community/app-badges?since=<iso>`. De **server** telt nieuwe **posts ÉN replies** in de community-feed + nieuwe gevolgde chatroom-topics, bepaalt zelf de admin-status (client stuurt geen `includeAllAuthors` meer voor de tijdlijn) en past de telregel toe: admin telt alle nieuwe posts+replies, gewone gebruiker enkel admin-geschreven posts+replies (+ gevolgde topics tellen altijd). Reden voor server-side: `community_posts` heeft géén `last_reply_at`, dus client-side reply-tellen zou N `listReplies`-calls vereisen. Vervaltermijn 6 weken + geblokkeerde auteurs zitten server-side. Bij fout → 0. **`countNewAdminChatroomActivity(since, includeAllAuthors?, topicReads?)` → `ChatroomActivity { total, perRoom, perTopic }`** blijft CLIENT-side (v3.1.0): per room `getRoom(slug)` telt **per topic** het nieuwe topic zelf `(includeAllAuthors \|\| author_is_admin) && created_at > effSince` PLUS nieuwe **replies** — voor topics met `last_reply_at > effSince` wordt `getTopic(id)` opgehaald en de replies geteld (zo blijft het aantal getTopic-calls beperkt tot recent-actieve topics). **`perTopic`** (keyed op topic.id) voedt de per-topic badges in `ChatRoomScreen` (locatie 4), `perRoom` = som per room (locatie 3), `total` = som over alles (footer). **Per-topic referentiepunt** `effSince` = het laatste van de globale baseline `since` en `topicReads[topicId]` (per-topic "gezien"-tijdstip uit `notif_topic_reads_<email>`), via `effectiveSince()`. **Vervaltermijn**: `withExpiry(since)` zet een ondergrens van **6 weken** (`BADGE_MAX_AGE_MS`) op de baseline in de chatruimte-teller (voor de tijdlijn zit diezelfde ondergrens server-side). `includeAllAuthors` (v2.13.1) = admin-modus, enkel nog voor de chatruimte-teller. Beide defensief: bij fout → 0 / lege maps (poll-loop crasht nooit). Wordt gepolld door `NotificationContext`. |
...
| `…/api/community/app-badges?since=` GET | Server-side tijdlijn-badge-teller → `{ timeline }` (nieuwe posts+replies+gevolgde topics, admin-logica + 6-weken-vervaltermijn server-side) | `services/community.ts::getAppBadges` (v3.1.x) |
...
| `…/api/community/app-badges?since=` GET | Server-side tijdlijn-badge-teller (posts+replies+gevolgde topics) | `services/community.ts::getAppBadges` → `notifications.ts::fetchTimelineBadge` |
...
6. 🟡 **Notificaties** — admin-post badges op de footer-tabs.
   ...
   - ✅ v3.2.0: **app-icoon-push (Expo Push, server-side trigger)**. De rode badge op het app-icoon (home-screen, ook wanneer de app dicht is) = ALLES opgeteld (tijdlijn + chatruimtes), zelfde admin/gevolgd-telregel. Werking:
     ...
     - ⬜ Nog nodig vóór productie: `expo-notifications` config in `app.json` (iOS `aps-environment`, Android-kanaal, plugin) + EAS-credentials (APNs-key + FCM) + nieuwe store-build. "Code eerst, credentials later" — de code is klaar, de credentials/build volgen.
