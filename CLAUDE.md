...
6. 🟡 **Notificaties** — admin-post badges op de footer-tabs.
   ...
   - ✅ v3.1.0: chatruimte-badges effectief tonen, tot op topic-niveau, met vervaltermijn:
     ...
     - **Vervaltermijn 6 weken** — `withExpiry()` in `services/notifications.ts` zet een ondergrens (`BADGE_MAX_AGE_MS`) op de baseline van zowel de tijdlijn- als de chatruimte-teller: items ouder dan 6 weken tellen nooit nog als nieuw, ook niet als de tab/het topic nooit geopend werd → badges lopen niet eindeloos op.
